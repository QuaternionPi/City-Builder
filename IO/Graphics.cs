...
public readonly struct Camera
{
    ...
    public static implicit operator Raylib_cs.Camera2D(Camera camera) => new Raylib_cs.Camera2D(SystemVector(camera.Offset), SystemVector(camera.Target), camera.Rotation, camera.Zoom);
    public readonly Vector2 Offset;
    public readonly Vector2 Target;
    public readonly float Rotation;
    public readonly float Zoom;
    public Camera(Vector2 offset, Vector2 target, float rotation, float zoom)
    {
        Offset = offset;
        Target = target;
        Rotation = rotation;
